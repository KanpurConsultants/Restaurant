...
#End Region
    ...
    Private Sub FrmShade_BaseEvent_Data_Validation(ByRef passed As Boolean) Handles Me.BaseEvent_Data_Validation
        ...
        If Topctrl1.Mode = "Add" Then
            mQry = "Select count(*) From Ht_Table Where Description='" & TxtDescription.Text & "' "
            If AgL.Dman_Execute(mQry, AgL.GCn).ExecuteScalar > 0 Then Err.Raise(1, , "Description Already Exists")

            mQry = "Select count(*) From Ht_Table_log Where Description ='" & TxtDescription.Text & "' And EntryStatus='" & ClsMain.LogStatus.LogOpen & "' and IsNull(MoveToLog,'')=''  "
            If AgL.Dman_Execute(mQry, AgL.GCn).ExecuteScalar > 0 Then Err.Raise(1, , "Description Already Exists in Log File")
        Else
            mQry = "Select count(*) From Ht_Table Where Description='" & TxtDescription.Text & "' And Code<>'" & mInternalCode & "' "
            If AgL.Dman_Execute(mQry, AgL.GCn).ExecuteScalar > 0 Then Err.Raise(1, , "Description Already Exists")

            mQry = "Select count(*) From Ht_Table_log Where Description='" & TxtDescription.Text & "' And UID <>'" & mSearchCode & "' And EntryStatus='" & ClsMain.LogStatus.LogOpen & "' and IsNull(MoveToLog,'')=''  "
            If AgL.Dman_Execute(mQry, AgL.GCn).ExecuteScalar > 0 Then Err.Raise(1, , "Description Already Exists in Log File")
        End If
    End Sub
    ...
    Private Sub FrmShade_BaseEvent_Form_PreLoad() Handles Me.BaseEvent_Form_PreLoad
        MainTableName = "Ht_Table"
        LogTableName = "Ht_Table_LOG"
    ...
    Private Sub FrmQuality1_BaseFunction_MoveRec(ByVal SearchCode As String) Handles Me.BaseFunction_MoveRec
        ...
        If FrmType = ClsMain.EntryPointType.Main Then
            mQry = "Select * " & _
                " From Ht_Table Where Code='" & SearchCode & "'"
        Else
            mQry = "Select * " & _
                " From Ht_Table_Log Where UID='" & SearchCode & "'"
        End If
