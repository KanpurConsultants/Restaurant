...
#End Region
    ...
    Private Sub FrmYarn_BaseEvent_Data_Validation(ByRef passed As Boolean) Handles Me.BaseEvent_Data_Validation
        ...
        If Topctrl1.Mode = "Add" Then
            mQry = "Select count(*) From Outlet Where Description='" & TxtDescription.Text & "' And " & AgTemplate.ClsMain.RetDivFilterStr & "  "
            If AgL.Dman_Execute(mQry, AgL.GCn).ExecuteScalar > 0 Then Err.Raise(1, , "Description Already Exist!")

            mQry = "Select count(*) From Outlet_Log Where Description='" & TxtDescription.Text & "' And " & AgTemplate.ClsMain.RetDivFilterStr & "  And EntryStatus='" & ClsMain.LogStatus.LogOpen & "' and IsNull(MoveToLog,'')=''  "
            If AgL.Dman_Execute(mQry, AgL.GCn).ExecuteScalar > 0 Then Err.Raise(1, , "Description Already Exists in Log File")
        Else
            mQry = "Select count(*) From Outlet Where Description='" & TxtDescription.Text & "' And Code<>'" & mInternalCode & "' And " & AgTemplate.ClsMain.RetDivFilterStr & "  "
            If AgL.Dman_Execute(mQry, AgL.GCn).ExecuteScalar > 0 Then Err.Raise(1, , "Description Already Exist!")

            mQry = "Select count(*) From Outlet_Log Where Description = '" & TxtDescription.Text & "' And UID <>'" & mSearchCode & "' And " & AgTemplate.ClsMain.RetDivFilterStr & "  And EntryStatus='" & ClsMain.LogStatus.LogOpen & "' and IsNull(MoveToLog,'')=''  "
            If AgL.Dman_Execute(mQry, AgL.GCn).ExecuteScalar > 0 Then Err.Raise(1, , "Description Already Exists in Log File")
        End If
    End Sub
    ...
    Public Overridable Sub FrmYarn_BaseEvent_FindLog() Handles Me.BaseEvent_FindLog
        ...
        AgL.PubFindQry = "SELECT I.UID, I.Description  " & _
                        " FROM Outlet_Log I " & mConStr & _
    ...
    Private Sub FrmOutlet_Load(ByVal sender As Object, ByVal e As System.EventArgs) Handles Me.Load
